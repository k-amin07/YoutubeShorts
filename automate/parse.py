...
class data:
    ...
    def add_value(self,attribute,value):
        if hasattr(self, attribute):
            if(attribute == "video_format"):
                self.resolution,self.frame_rate = re.findall(r'(\d+x\d+)@(\d+)',value)[0]
    ...
    # Get class attributes in the order they are defined
        attribute_values = [str(getattr(self, attr)) for attr in ['device_info', 'cpn', 'video_id', 'video_format', 'resolution', 'frame_rate', 'audio_format', 'volume', 'bandwidth_estimate', 'readahead', 'viewport', 'dropped_frames', 'mystery_text']]
